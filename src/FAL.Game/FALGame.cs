...
namespace FAL.Game
{
    public class FALGame
    {
        ...
        public async Task<(int, int, int)> Play()
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                await _players[i].SendAsync(ServerOperationType.GameStart);
                await _players[i].SendAsync(ServerOperationType.GameInformation, new GameInformation(_players.Select(p => (p.Name, p.Score)).ToArray()));
                _players[i].Cards = new List<int>(playerCards[i]);
                await _players[i].SendAsync(ServerOperationType.UpdateCards, _players[i].Cards);
            }
            return default;
        }
